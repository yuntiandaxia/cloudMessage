...
NSString * const YTMenuWillAppearNotification    = @"YTMenuWillAppearNotification";
NSString * const YTMenuDidAppearNotification     = @"YTMenuDidAppearNotification";
NSString * const YTMenuWillDisappearNotification = @"YTMenuWillDisappearNotification";
NSString * const YTMenuDidDisappearNotification  = @"YTMenuDidDisappearNotification";
...
// 背景色效果
static YTMenuBackgrounColorEffect gBackgroundColorEffect = YTMenuBackgrounColorEffectSolid;
// 是否显示阴影
...
typedef enum {
    YTMenuViewArrowDirectionNone,
    YTMenuViewArrowDirectionUp,
    YTMenuViewArrowDirectionDown,
    YTMenuViewArrowDirectionLeft,
    YTMenuViewArrowDirectionRight,
} YTMenuViewArrowDirection;

@interface YTMenuView : UIView
...
//菜单的底层
@interface YTMenuOverlay : UIView
...
#pragma
#pragma mark - YTMenuOverlay
#pragma

@implementation YTMenuOverlay
...
- (void)singleTap:(UITapGestureRecognizer *)recognizer {
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[YTMenuView class]] && [view respondsToSelector:@selector(dismissMenu:)]) {
...
#pragma
#pragma mark - YTMenuView
#pragma 

@implementation YTMenuView {
    YTMenuViewArrowDirection _arrowDirection;
    CGFloat _arrowPosition;
    UIView  *_contentView;
    NSArray *_menuItems;
    YTMenuSelectedItem _selectedItem;
}
...
- (id)init {
    ...
    if(self) {
        ...
        if ([YTMenu hasShadow]) {
            ...
            self.layer.shadowColor   = [YTMenu tintColor].CGColor;
...
- (void)setupFrameInView:(UIView *)view fromRect:(CGRect)fromRect {
    ...
    const CGFloat widthPlusArrow  = contentSize.width  + [YTMenu arrowSize];
    const CGFloat heightPlusArrow = contentSize.height + [YTMenu arrowSize];
    ...
    if (heightPlusArrow < (outerHeight - rectY1)) {
        ...
        _arrowDirection = YTMenuViewArrowDirectionUp;
        ...
        _contentView.frame = (CGRect){0, [YTMenu arrowSize], contentSize};
        
        self.frame = (CGRect) {
            point,
            contentSize.width,
            contentSize.height + [YTMenu arrowSize]
        };
        
    } else if (heightPlusArrow < rectY0) {
        
        _arrowDirection = YTMenuViewArrowDirectionDown;
        ...
        self.frame = (CGRect) {point,contentSize.width,contentSize.height + [YTMenu arrowSize]};
        
    } else if (widthPlusArrow < (outerWidth - rectX1)) {
        
        _arrowDirection = YTMenuViewArrowDirectionLeft;
        ...
        _contentView.frame = (CGRect){[YTMenu arrowSize], 0, contentSize};
        
        self.frame = (CGRect) {point,contentSize.width + [YTMenu arrowSize],contentSize.height};
        
    } else if (widthPlusArrow < rectX0) {
        
        _arrowDirection = YTMenuViewArrowDirectionRight;
        ...
        self.frame = (CGRect) {
            ...
            point,
            contentSize.width  + [YTMenu arrowSize],
            contentSize.height
        };
        
    } else {
        
        _arrowDirection = YTMenuViewArrowDirectionNone;
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(YTMenuSelectedItem)selectedItem
{
    ...
    YTMenuOverlay *overlay = [[YTMenuOverlay alloc] initWithFrame:view.bounds];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:YTMenuWillAppearNotification object:nil];
    ...
    [UIView animateWithDuration:0.2 animations:^(void) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:YTMenuDidAppearNotification object:nil];
    }];
}

- (void)dismissMenu:(BOOL)animated {
    if (self.superview) {
        [[NSNotificationCenter defaultCenter] postNotificationName:YTMenuWillDisappearNotification object:nil];
        
        __weak typeof(self) weakSelf = self;
        void (^removeView)(void) = ^(void) {
            if ([weakSelf.superview isKindOfClass:[YTMenuOverlay class]])
                [weakSelf.superview removeFromSuperview];
            [weakSelf removeFromSuperview];
            
            [[NSNotificationCenter defaultCenter] postNotificationName:YTMenuDidDisappearNotification object:nil];
        };
    ...
    [YTMenu sharedMenu].isShow = NO;
}

- (void)performAction:(id)sender {
    [self dismissMenu:YES];
    UIButton *button = (UIButton *)sender;
    YTMenuItem *menuItem = _menuItems[button.tag];
...
- (UIView *)mkContentView {
    ...
    UIFont *titleFont = [YTMenu titleFont];
    
    CGFloat maxImageWidth = 0;
    CGFloat maxItemHeight = 0;
    CGFloat maxItemWidth  = 0;
    
    for (YTMenuItem *menuItem in _menuItems) {
    ...
    for (YTMenuItem *menuItem in _menuItems) {
        ...
        const CGFloat itemHeight = MAX(titleSize.height, imageSize.height) + [YTMenu menuItemMarginY] * 2;
    ...
    UIImage *selectedImage = [YTMenuView selectedImage:(CGSize){maxItemWidth, maxItemHeight}];
    
    UIImage *gradientLine = [YTMenuView gradientLineWithColor:[YTMenu separatorColor]];
    ...
    for (YTMenuItem *menuItem in _menuItems) {
        ...
        if (menuItem.title.length) {
            ...
            if (!menuItem.enabled && !menuItem.image) {
                ...
                titleFrame = (CGRect){
                    kMarginX * 2,
                    [YTMenu menuItemMarginY],
                    maxItemWidth - kMarginX * 4,
                    maxItemHeight - [YTMenu menuItemMarginY] * 2
                };
        ...
        if (menuItem.image) {
            ...
            const CGRect imageFrame = {kMarginX * 2, [YTMenu menuItemMarginY], maxImageWidth, maxItemHeight - [YTMenu menuItemMarginY] * 2};
...
- (CGPoint)arrowPoint {
    ...
    if (_arrowDirection == YTMenuViewArrowDirectionUp) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMinY(self.frame) };
        
    } else if (_arrowDirection == YTMenuViewArrowDirectionDown) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMaxY(self.frame) };
        
    } else if (_arrowDirection == YTMenuViewArrowDirectionLeft) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame), CGRectGetMinY(self.frame) + _arrowPosition  };
        
    } else if (_arrowDirection == YTMenuViewArrowDirectionRight) {
...
+ (UIImage *)selectedImage:(CGSize)size {
    ...
    CGContextSetFillColorWithColor(context, [[YTMenu selectedColor] CGColor]);
...
- (void)drawBackground:(CGRect)frame inContext:(CGContextRef) context {
    ...
    UIColor *tintColor = [YTMenu tintColor];
    if (tintColor) {
        CGFloat a;
    ...
    if ([YTMenu backgrounColorEffect] == YTMenuBackgrounColorEffectGradient) {
        R1-=0.2;
        G1-=0.2;
        B1-=0.2;
    }
    ...
    if (_arrowDirection == YTMenuViewArrowDirectionUp) {
        ...
        const CGFloat arrowX0 = arrowXM - [YTMenu arrowSize];
        const CGFloat arrowX1 = arrowXM + [YTMenu arrowSize];
        const CGFloat arrowY0 = Y0;
        const CGFloat arrowY1 = Y0 + [YTMenu arrowSize] + kEmbedFix;
        ...
        const CGFloat arrowX0 = arrowXM - [YTMenu arrowSize];
        const CGFloat arrowX1 = arrowXM + [YTMenu arrowSize];
        const CGFloat arrowY0 = Y1 - [YTMenu arrowSize] - kEmbedFix;
        ...
        const CGFloat arrowX1 = X0 + [YTMenu arrowSize] + kEmbedFix;
        const CGFloat arrowY0 = arrowYM - [YTMenu arrowSize];;
        const CGFloat arrowY1 = arrowYM + [YTMenu arrowSize];
        ...
        const CGFloat arrowX1 = X1 - [YTMenu arrowSize] - kEmbedFix;
        const CGFloat arrowY0 = arrowYM - [YTMenu arrowSize];;
        const CGFloat arrowY1 = arrowYM + [YTMenu arrowSize];
    ...
    UIBezierPath *borderPath = [UIBezierPath bezierPathWithRoundedRect:bodyFrame
                                                          cornerRadius:[YTMenu cornerRadius]];
    ...
    if (_arrowDirection == YTMenuViewArrowDirectionLeft ||
        _arrowDirection == YTMenuViewArrowDirectionRight) {
...
@implementation YTMenu{
    YTMenuView *_menuView;
    BOOL        _observing;
}
...
+ (void)reset {
    ...
    gBackgroundColorEffect = YTMenuBackgrounColorEffectSolid;
    gHasShadow             = NO;
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(YTMenuSelectedItem)selectedItem {
    ...
    // 创建MenuView
    _menuView = [[YTMenuView alloc] init];
